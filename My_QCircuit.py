...
def qc_Hidden_Layer2(prg, MCRY = True):
    ...
    if MCRY == True:
        ...
        # interference
        theta_z = 2*np.arccos(1/np.sqrt(3))
        f_theta_z = 2*np.arccos(1/3) + 2*np.pi
        
        
        qc += X.on(2)
        qc += X.on(3)
        qc += Z.on(0,[1,2,3])
        qc += RY(f_theta_z).on(4,[0,1,2,3])
        qc += X.on(2)
        qc += X.on(3)
        
        qc += RY(theta_z).on(4,[0,1])
        qc += X.on(2)
        qc += X.on(3)
        qc += RY(theta_z).on(5,[2,3])
        qc += X.on(2)
        qc += X.on(3)
...
def qc_chip(Enc1 = True, Enc2 = True, Hidden_1 = True, Hidden_2 = True, MCRY = True, tomography = True):
    """
    Function:
